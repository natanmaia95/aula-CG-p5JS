...
function drawLineBre(x1, y1, x2, y2) {
  ...
  for (let currentX = x1; currentX <= x2; currentX) {
    ...
    currentX += 1;
    distance += a;
    candidateDistance = distance + b;
    //decide if Y should increase or not
    if (abs(candidateDistance) < abs(distance)) {
      currentY += 1;
...
function draw() {
  ...
  drawLineBre(floor(originX/20),floor(originY/20), mousePixelX, mousePixelY)
